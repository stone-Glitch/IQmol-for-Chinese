@echo off
REM =============================================================================
REM  IQmol-for-Chinese  Windows 一键构建脚本
REM
...
REM      mingw-w64-x86_64-zlib mingw-w64-x86_64-hdf5 mingw-w64-x86_64-fortran
REM
...
if not errorlevel 1 (
    ...
    lrelease "%ROOT%\translations\zh_CN.ts" -qm "%BUILD%\translations\zh_CN.qm"
)
...
if exist "%ROOT%\modules\openbabel\data" (
    if not exist "%~dp0%EXE%\..\share" mkdir "%~dp0%EXE%\..\share"
    xcopy /e /i /y "%ROOT%\modules\openbabel\data" "%~dp0%EXE%\..\share\openbabel" >nul
    echo [OK] OpenBabel 数据已复制
)
...
echo  构建完成！
echo  可执行文件: %EXE%
echo  分发时请将 exe 同目录的所有 DLL/文件夹一并打包。
echo ============================================================
endlocal
