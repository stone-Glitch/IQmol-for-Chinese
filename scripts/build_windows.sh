...
# 注意: libssh2 1.11 起无 src/libssh2.c(拆分为 session.c/global.c 等),
# 完整性标志用 CMakeLists.txt + src/session.c + include/libssh2.h。
if [ ! -f "$MODULES_DIR/libssh2/CMakeLists.txt" ] || [ ! -f "$MODULES_DIR/libssh2/src/session.c" ] || [ ! -f "$MODULES_DIR/libssh2/include/libssh2.h" ]; then
  echo "ERROR: modules/libssh2 源码缺失或不完整(需要 CMakeLists.txt、src/session.c、include/libssh2.h)" >&2
...
# 判定标志: configure 生成阶段会为每个 target 写出
# modules/libssh2/CMakeFiles/libssh2_static.dir/build.make, 缺它即说明
# 构建树里没有 libssh2 target → touch 顶层 CMakeLists.txt 令 make 自动重配。
if [ -f "$BUILD_DIR/CMakeCache.txt" ] && [ -f "$BUILD_DIR/Makefile" ] \
   && [ ! -f "$BUILD_DIR/modules/libssh2/CMakeFiles/libssh2_static.dir/build.make" ]; then
