...
> - 译文条数 **78 → 1519 → 2111（当前主分支）**（见 `2026-09-13-编译与运行验证.md`）；
...
| `translations/zh_CN.ts` | 2111 条中文译文（`lupdate` 提取 + 手工填充，0 unfinished；早期 78 条 → 1519 → 2111，随全量 UI 扩展与审计修正递增） |
...
2. **qm 生成**：构建期 `lrelease` 输出 `Generated 2111 translation(s) (2111 finished and 0 unfinished)`（历史：早期 78 条 → 1519 → 2111）。
