# 发布与上传：受限网络下推送 GitHub 的可行通道

**日期**: 2026-09-05（2026-09-25 归档整理）
**场景**: WorkBuddy 沙箱（Ubuntu 22.04）无法用常规方式推送 GitHub 时的可行通道
**实测环境**: 本项目仓库 `stone-Glitch/IQmol-for-Chinese`

> 本文是**受限网络环境下**的推送/上传方案。常规环境直接 `git push` 即可，无需阅读。
> 构建与打包流程见 [`构建与部署指南.md`](构建与部署指南.md)；
> 本仓库当前默认分支为 `main`（历史上的 `master` 已并入，见文末第八节）。
