...
void CameraDialog::on_addFrameButton_clicked(bool)
{
   ...
   QVariant v(QVariantPtr<Frame>::toQVariant(cameraFrame));
...
void CameraDialog::on_playButton_clicked(bool)
{
   ...
   for (int row = 0; row < table->rowCount(); ++row) {
       ...
       Frame* frame = QVariantPtr<Frame>::toPointer(item->data(Qt::UserRole));
       if (frame && ok) {
...
void CameraDialog::runFrame()
{
   ...
   frame = QVariantPtr<Frame>::toPointer(item->data(Qt::UserRole));
