...
void SurfaceAnimatorDialog::update()
{
   ...
   for (iter = cubeFiles.begin(); iter != cubeFiles.end(); ++iter) {
       ...
       QVariant qvar(QVariantPtr<Layer::CubeData>::toQVariant(*iter)); 
...
void SurfaceAnimatorDialog::computeIsovalueAnimation()
{
   ...
   Layer::CubeData* cube;
   cube = QVariantPtr<Layer::CubeData>::toPointer(item->data(Qt::UserRole));
   if (!cube) return;
...
void SurfaceAnimatorDialog::computeMultiGridAnimation()
{
   ...
   QListWidgetItem* item(fileList->item(0));
   Layer::CubeData* cube(QVariantPtr<Layer::CubeData>::toPointer(item->data(Qt::UserRole)));
